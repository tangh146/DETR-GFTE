...
from utils import get_psuedo_knn

class GraphNetwork(nn.Module):
    def __init__(self, d_model, num_classes):
        super(GraphNetwork, self).__init__()
        self.conv1 = GCNConv(256, d_model)
...
class GNLightning(pl.LightningModule):
    ...
    def forward(self, pred_bboxes, hidden_states):
        # bboxes tensor of shape (num_bboxes, 4) in coco format
        # orig size tensor of shape (2)

        edge_index = get_psuedo_knn(pred_bboxes)

        data = Data(x=hidden_states, edge_index=edge_index).to(self.device)
